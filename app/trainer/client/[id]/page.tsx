...
export default function ClientCRMPage() {
  ...
  // Список упражнений из базы (для поиска)
  ...
  useEffect(() => {
      if (showLibrary) loadTemplates();
  }, [showLibrary]);

  // Загружаем список упражнений при старте
  useEffect(() => {
      fetch('/api/trainer/exercises')
        .then(res => res.json())
        .then(data => { if(Array.isArray(data)) setExerciseList(data); });
  }, []);

  const loadTemplates = () => {
      fetch('/api/trainer/get-templates')
        .then(res => res.json())
  ...
  // --- НОВАЯ ФУНКЦИЯ: КЛОНИРОВАТЬ ИЗ ИСТОРИИ ---
  const handleCloneFromHistory = async (log: any) => {
      if (!confirm('Скопировать эту тренировку в текущий план?')) return;
      setIsProcessing(true);

      try {
          // 1. Разбираем старую тренировку
          const oldExercises = JSON.parse(log.details);
          
          // 2. Чистим её (убираем выполненные подходы, оставляем структуру)
          const newExercises = oldExercises.map((ex: any) => {
              // Пытаемся достать вес, с которым делали, чтобы поставить его как план
              let suggestedWeight = ex.weight || '';
              if (ex.actualSets && ex.actualSets.length > 0) {
                  suggestedWeight = ex.actualSets[0].weight || '';
              } else if (ex.doneWeight) {
                  suggestedWeight = ex.doneWeight;
              }
              ...
              return {
                  ...
                  sets: ex.sets || '3',
                  reps: ex.reps || '10',
                  weight: suggestedWeight // Подставляем прошлый вес как план
              };
          });

          // 3. Создаем новую тренировку через API назначения
          const dateStr = new Date(log.date).toLocaleDateString('ru-RU');
          await fetch('/api/programs/assign', {
              method: 'POST',
              body: JSON.stringify({
                  clientId,
                  name: `Повтор от ${dateStr}`, // Название новой тренировки
                  exercises: newExercises
              })
          });

          // 4. Переключаемся на вкладку Плана, чтобы увидеть результат
          alert('Тренировка скопирована в План!');
          loadData();
          setActiveTab('plan');
  ...
  const handleCreateExercise = async (name: string) => {
      if (!name) return;
      setExerciseList(prev => [...prev, { name }].sort((a,b) => a.name.localeCompare(b.name)));
      try { await fetch('/api/trainer/exercises', { method: 'POST', body: JSON.stringify({ name }) }); } catch (e) { console.error(e); }
  };
  ...
  const startPlannedWorkout = async (workout: any) => { try { await fetch('/api/programs/assign', { method: 'POST', body: JSON.stringify({ clientId, name: workout.name, split: 'Auto', exercises: JSON.parse(workout.exercises) }) }); router.push(`/trainer/workout/${clientId}`); } catch (e) { alert("Ошибка."); } };
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-4 pb-32 relative" onClick={() => setActiveSearchIndex(null)}>
                ...
                return (
                    <div key={log.id} className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden">
                        <div className="bg-gray-700/30 p-3 border-b border-gray-700 flex justify-between items-center">
                            ...
                            <button 
                                onClick={() => handleCloneFromHistory(log)}
                                className="text-[10px] bg-blue-900/40 text-blue-300 px-3 py-1 rounded border border-blue-500/30 hover:bg-blue-800 hover:text-white transition"
                            >
                                ↺ Повторить
                            </button>
                        </div>
                        <div className="p-4 space-y-3">
                            {exercises.map((ex: any, idx: number) => {
                                ...
                                return (
                                    <div key={idx} className="flex justify-between items-start text-sm border-b border-gray-700/30 pb-2 last:border-0 last:pb-0">
                                        <span className="text-gray-300 w-2/3">{ex.name}</span>
                                        <div className="text-right"><div className="font-bold text-white">{weight ? weight+' кг' : '-'}</div><div className="text-xs text-gray-500">{setsInfo}</div></div>
                                    </div>
                                );
                            })}
                        </div>
      ...
      {/* ОСТАЛЬНЫЕ МОДАЛКИ (Без изменений, вставь код из прошлого шага) */}
